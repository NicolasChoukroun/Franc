...
static constexpr int64_t STALE_CHECK_INTERVAL = 3600*24; // 10 minutes
...
static constexpr int STALE_RELAY_AGE_LIMIT = 3600*24;
/// Age after which a block is considered historical for purposes of rate
/// limiting block relay. Set to one week, denominated in seconds.
static constexpr int HISTORICAL_BLOCK_AGE = 7 * 24 * 60 * 60 * 30;
    ...
    while (!done && !orphan_work_set.empty()) {
        ...
        if (AcceptToMemoryPool(mempool, orphan_state, porphanTx, &fMissingInputs2, &removed_txn, false /* bypass_limits */, 0 /* nAbsurdFee */)) {
            ...
            if (!orphanTx.HasWitness() && orphan_state.GetReason() != ValidationInvalidReason::TX_WITNESS_MUTATED) {
                ...
                // See https://github.com/franc/franc/issues/8279 for details.
                assert(recentRejects);
