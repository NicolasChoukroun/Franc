This directory contains the francQT graphical user interface (GUI). It uses the cross-platform framework [Qt](https://www1.qt.io/developers/).
...
Represents the main window of the franc UI.
...
ClientModel is used by the main application `francgui` and several models like `peertablemodel`.
...
Used to process BIP21 and BIP70 (see https://github.com/franc/franc/pull/11622) payment URI / requests. Also handles URI based application switching (e.g. when following a franc:... link from a browser).
...
* UI elements like francAmountField, which inherit from QWidget.
* `francstrings.cpp`: automatically generated
* `francunits.(h/cpp)`: franc / mfranc / etc handling
* `callback.h`
...
* don't change `local/franc_en.ts`; this happens [automatically](/doc/translation_process.md#writing-code-with-translations)
...
10. Start debugging with Qt Creator (you might need to the executable to "franc-qt" under "Run", which is where you can also add command line arguments)
