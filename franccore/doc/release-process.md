...
* Update `src/chainparams.cpp` chainTxData with statistics about the transaction count and rate. Use the output of the RPC `getchaintxstats`, see
  [this pull request](https://github.com/franc/franc/pull/17002) for an example. Reviewers can verify the results by running `getchaintxstats <window_block_count> <window_last_block_hash>` with the `window_block_count` and `window_last_block_hash` from your output.
* On both the master branch and the new release branch:
  - update `CLIENT_VERSION_MINOR` in [`configure.ac`](../configure.ac)
  - update `CLIENT_VERSION_MINOR`, `PACKAGE_VERSION`, and `PACKAGE_STRING` in [`build_msvc/franc_config.h`](/build_msvc/franc_config.h)
* On the new release branch in [`configure.ac`](../configure.ac) and [`build_msvc/franc_config.h`](/build_msvc/franc_config.h) (see [this commit](https://github.com/franc/franc/commit/742f7dd)):
...
Open a draft of the release notes for collaborative editing at https://github.com/franc-core/franc-devwiki/wiki.
...
### Build and sign Franc for Linux, Windows, and macOS:
...
franc-${VERSION}-riscv64-linux-gnu.tar.gz
...
- Update other repositories and websites for new version
  ...
  - franccore.org maintained versions update:
    [table](https://github.com/bitcoin-core/bitcoincore.org/commits/master/_includes/posts/maintenance-table.md)

  - franccore.org RPC documentation update
  ...
  - Update packaging repo
      ...
      - Notify MarcoFalke so that he can start building the snap package
        ...
        - https://code.launchpad.net/~franc-core/franc-core-snap/+git/packaging (Click "Import Now" to fetch the branch)
        - https://code.launchpad.net/~franc-core/franc-core-snap/+git/packaging/+ref/0.xx (Click "Create snap package")
        - Name it "franc-core-snap-0.xx"
        - Leave owner and series as-is
        - Select architectures that are compiled via gitian
        - Leave "automatically build when branch changes" unticked
        - Tick "automatically upload to store"
        - Put "franc-core" in the registered store package name field
        - Tick the "edge" box
        - Put "0.xx" in the track field
        - Click "create snap package"
        - Click "Request builds" for every new release on this branch (after updating the snapcraft.yml in the branch to reflect the latest gitian results)
        - Promote release on https://snapcraft.io/franc-core/releases if it passes sanity checks
  ...
  - This repo
      ...
      - Create a pinned meta-issue for testing the release candidate (see [this issue](https://github.com/franc/franc/issues/15555) for an example) and provide a link to it in the release announcements where useful

- Announce the release:

  - franc-dev and franc-core-dev mailing list

  - Franc announcements list https://franccore.org/en/list/announcements/join/
