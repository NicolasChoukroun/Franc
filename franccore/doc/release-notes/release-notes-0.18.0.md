Franc version 0.18.0 is now available from:

  <https://franccore.org/bin/franc-core-0.18.0/>
...
`/Applications/franc-Qt` (on Mac) or `francd`/`franc-qt` (on
Linux).
...
Franc is supported and extensively tested on operating systems
using the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not
recommended to use Franc on unsupported systems.
...
10.10. Additionally, Franc does not yet change appearance when
...
`contrib/init/francd.service`, it has been changed to use
`/var/lib/francd` as the data directory instead of
`~franc/.franc`. When switching over to the new configuration file,
please make sure that the filesystem on which `/var/lib/francd` will
exist has enough space (check using `df -h /var/lib/francd`), and
...
- Four new options are available for configuring the maximum number of
  ...
  documentation](https://github.com/franc/franc/blob/master/doc/zmq.md#usage)
  for details.
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](https://franccore.org/en/contact/) the Franc
...
The systemd init file (`contrib/init/francd.service`) has been changed
to use `/var/lib/francd` as the data directory instead of
`~franc/.franc`. This change makes Franc more consistent with
...
`datadir` option in `/etc/franc/franc.conf` will have no effect.
This is because the command line arguments specified in the init files
take precedence over the options specified in
`/etc/franc/franc.conf`.


Documentation
-------------

- A new short [document](https://github.com/franc/franc/blob/master/doc/JSON-RPC-interface.md)
  about the JSON-RPC interface describes cases where the results of an
  RPC might contain inconsistencies between data sourced from different
  subsystems, such as wallet state and mempool state.  A note is added
  to the [REST interface documentation](https://github.com/franc/franc/blob/master/doc/REST-interface.md)
  indicating that the same rules apply.

- Further information is added to the [JSON-RPC
  documentation](https://github.com/franc/franc/blob/master/doc/JSON-RPC-interface.md)
  about how to secure this interface.

- A new [document](https://github.com/franc/franc/blob/master/doc/franc-conf.md)
  about the `franc.conf` file describes how to use it to configure
  Franc.

- A new document introduces Franc's BIP174 [Partially-Signed
  franc Transactions
  (PSBT)](https://github.com/franc/franc/blob/master/doc/psbt.md)
...
- The [output script
  descriptor](https://github.com/franc/franc/blob/master/doc/descriptors.md)
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent franc-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  Franc to libssl vulnerabilities in the past, builders who don't
...
- The minimum required version of Qt (when building the GUI) has been
  ...
  system](https://github.com/franc/franc/blob/master/depends/README.md)
  provides 5.9.7)
...
- `deriveaddresses` returns one or more addresses corresponding to an
  [output descriptor](https://github.com/franc/franc/blob/master/doc/descriptors.md).
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when Franc knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of Franc, subsequently importing
...
- `importmulti` now supports importing of addresses from descriptors. A
  ...
  [here](https://github.com/franc/franc/blob/master/doc/descriptors.md).
...
- The 'account' API is removed after being deprecated in v0.17.  The
  ...
  v0.17](https://github.com/franc/franc/blob/master/doc/release-notes/release-notes-0.17.0.md#label-and-account-apis-for-wallet)
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart francd with the `-deprecatedrpc=generate`
...
- A new `franc-wallet` tool is now distributed alongside franc
...
This section describes planned changes to Franc that may affect
other franc software and services.

- Since version 0.16.0, Franc’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with Franc 0.20 (expected about
  a year after 0.18), Franc will default to native segwit
  ...
  to bech32 addresses, and if the Franc project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in Franc 0.19 (approximately November 2019).
  ...
  set the `addresstype=bech32` configuration option in any Franc
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, Franc
...
- A sub-project of Franc now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with Franc.  See their [project
  page](https://github.com/franc-core/HWI#readme) for details.

Security
--------

- This release changes the Random Number Generator (RNG) used from
  OpenSSL to Franc's own implementation, although entropy
  gathered by Franc is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves Franc a
...
- On macOS, Franc now opts out of application CPU throttling
...
- #12490 Remove deprecated wallet rpc features from `franc_server` (jnewbery)
- #14138 Set `encrypted_batch` to nullptr after delete. Avoid double free in the case of NDEBUG (practicalswift)
- #14168 Remove `ENABLE_WALLET` from `libfranc_server.a` (jnewbery)
...
- #13926 [Tools] franc-wallet - a tool for creating and managing wallets offline (jnewbery)
...
- #14162 Also log and print messages or questions like francd (MarcoFalke)
...
- #14521 qt, docs: Fix `franc-qt -version` output formatting (hebasto)
...
- #11625 Add francApplication & RPCConsole tests (ryanofsky)
...
- #14204 Move `interfaces/*` to `libfranc_server` (laanwj)
...
- #15154 Configure: franc-tx doesn't need libevent, so don't pull it in (luke-jr)
...
- #15388 Makefile.am: add rule for src/franc-wallet (Sjors)
...
- #11526 Visual Studio build configuration for Franc (sipsorcery)
- #15110 build\_msvc: Fix the build problem in `libfranc_server` (Mr-Leshiy)
...
- #13953 Fix deprecation in franc-util-test.py (isghe)
...
- #12775 Integration of property based testing into Franc (Christewart)
...
- #12246 Bugfix: Only run franc-tx tests when franc-tx is enabled (luke-jr)
...
- #14092 Dry run `bench_franc` as part `make check` to allow for quick identification of assertion/sanitizer failures in benchmarking code (practicalswift)
...
- #14985 Remove `thread_local` from `test_franc` (MarcoFalke)
...
- #14057 logging: Only log `using config file path_to_franc.conf` message on startup if conf file exists (leishman)
...
- #12255 Update franc.service to conform to init.md (dongcarl)
...
- #14497 Add doc/franc-conf.md (hebasto)
...
- #15354 Add missing `franc-wallet` tool manpages (MarcoFalke)
...
- #15765 correct francconsensus_version in shared-libraries.md (fanquake)
- #15792 describe onlynet option in doc/tor.md (jonatack)
- #15802 mention creating application support franc folder on OSX (JimmyMow)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/projects/p/franc/).
