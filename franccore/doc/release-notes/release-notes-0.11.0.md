Franc version 0.11.0 is now available from:

  <https://franc.org/bin/franc-core-0.11.0/>
...
defaults to 0.00001. This will cause transactions with fewer franc/kB fee to be
...
For example, add the following to `franc.conf`:
...
related to the blockchain in the franc system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows Franc to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that Franc will 
...
For an example script see [franc-submittx](https://github.com/laanwj/franc-submittx).
...
- #5334 `cf87536` libfrancconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new franc-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only francAmountField
- #5753 `0f44672` Add franc logo to about screen
...
- #5809 `46bfbe7` Add franc-cli man page
...
- #5849 `41259ca` contrib/init/francd.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename franc-Qt.app to franc-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for franc-tx)
