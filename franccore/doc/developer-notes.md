...
- [Development guidelines](#development-guidelines)
    - [General Franc](#general-franc-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Franc uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play francs" on the test network, if you
...
Franc is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/franc/franc/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_franc
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_franc --log_level=test_suite
$ valgrind -v --leak-check=full src/francd -printtoconsole
```
...
# A coverage report will now be accessible at `./test_franc.coverage/index.html`.
```
...
To profile a running francd process for 60 seconds, you could use an
invocation of `perf record` like this:

```sh
$ perf record \
    -g --call-graph dwarf --per-thread -F 140 \
    -p `pgrep francd` -- sleep 60
```
...
Franc can be compiled with various "sanitizers" enabled, which add
 ...
 * [Issue #12691: Enable -fsanitize flags in Travis](https://github.com/franc/franc/issues/12691)
...
However, in open source software such as Franc, where everyone uses
...
pay attention to for reviewers of Franc code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers.
  ...
  - *Rationale*: Franc uses tinyformat, which is type safe. Leave them out to avoid confusion.
...
```c++
#ifndef FRANC_FOO_BAR_H
#define FRANC_FOO_BAR_H
...
#endif // FRANC_FOO_BAR_H
```
...
Some of these are maintained by active developers of Franc, in which case changes should probably go
directly upstream without being PRed directly against the project. They will be merged back in the next
subtree merge.

Others are external projects without a tight relationship with our project. Changes to these should also
be sent upstream, but bugfixes may also be prudent to PR against Franc so that they can be integrated
...
- src/libsecp256k1
  - Upstream at https://github.com/franc-core/secp256k1/ ; actively maintained by Core contributors.

- src/crypto/ctaes
  - Upstream at https://github.com/franc-core/ctaes ; actively maintained by Core contributors.

- src/univalue
  - Upstream at https://github.com/franc-core/univalue ; actively maintained by Core contributors, deviates from upstream https://github.com/jgarzik/univalue
...
file descriptors, it will cause problems with franc's `select()` loop, because
...
compatibility between nodes. This happened in franc 0.8 (when LevelDB was
...
would be to revert the upstream fix before applying the updates to franc's
...
Commit [`bb81e173`](https://github.com/franc/franc/commit/bb81e173) is an example of a scripted-diff.
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `franc-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `franc-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of Franc, the wallet was always
