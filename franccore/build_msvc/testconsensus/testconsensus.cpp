...
int main()
{
    std::cout << "francconsensus version: " << francconsensus_version() << std::endl;
    ...
    auto op0Result = francconsensus_verify_script_with_amount(pubKeyScript.data(), pubKeyScript.size(), amount, (const unsigned char*)&stream[0], stream.size(), 0, francconsensus_SCRIPT_FLAGS_VERIFY_ALL, &err);
